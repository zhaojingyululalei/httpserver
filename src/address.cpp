...
//遍历所有网卡接口，只要是符合family的都要保存到result中
//网卡类型，{地址对象，网络地址长度}
/*result = {
    {"lo", {IPv4Address("127.0.0.1"), 8}},
    {"eth0", {IPv4Address("192.168.1.100"), 24}}
}; */
bool Address::GetInterfaceAddresses(
    ...
    try {
        ...
        dbg << "Address::GetInterfaceAddresses exception";
        freeifaddrs(results);
        return false;
    }
    freeifaddrs(results);
...
/**
 * 可以获取指定网卡的地址对象，如果iface为空或者为"*"，则返回所有网卡的
 * iface: 网卡名称，如"lo"、"eth0"
 * result = {
    {IPv4Address("192.168.1.100"), 24},
    {IPv6Address("fe80::1"), 64}
};
 */
...
//就是把字符串形式的"127.0.0.1"转换成sockaddr_in 结构
//getaddrinfo 把AI_NUMERICHOST去掉，就可以通过DNS系统，解析域名，"baidu.com"，一个域名可能对应
//多个ip，因此addrinfo是链表形式，他有next字段
...
IPv4Address::ptr IPv4Address::Create(const char* address, uint16_t port) {
    ...
    if(result <= 0) {
        dbg << "IPv4Address::Create(" << address << ", "
...
//prefix_len:网络地址长度   32-prefix_len:主机地址长度
IPAddress::ptr IPv4Address::broadcastAddress(uint32_t prefix_len) {
    ...
    baddr.sin_addr.s_addr |= byteswapOnLittleEndian(
            CreateMask<uint32_t>(prefix_len));//把主机部分全部置1，就是广播地址
...
IPv6Address::ptr IPv6Address::Create(const char* address, uint16_t port) {
    ...
    if(result <= 0) {
        dbg << "IPv6Address::Create(" << address << ", "
                << port << ") rt=" << result << " errno=" << errno
                << " errstr=" << strerror(errno);
        return nullptr;
    }
