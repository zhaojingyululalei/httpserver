...
void HttpRequest::init() {
    std::string conn = getHeader("connection"); //获取请求头connect字段
    if(!conn.empty()) {
        if(strcasecmp(conn.c_str(), "keep-alive") == 0) {
            m_close = false; //如果设置了keep-alive
        } else {
            m_close = true;
        }
    }
...
void HttpRequest::initQueryParam() {
    if(m_parserParamFlag & 0x1) { //已经解析过了，就跳过
        return;
    }
/**
 *  str 中按 = 提取键
...
#define PARSE_PARAM(str, m, flag, trim) \
    ...
    do { \
        ...
        m.insert(std::make_pair(trim(str.substr(last, key - last)), \
                    zhao::StringUtil::UrlDecode(str.substr(key + 1, pos - key - 1)))); \
        if(pos == std::string::npos) { \
            break; \
        } \
        ++pos; \
...
void HttpRequest::initCookies() {
    ...
    PARSE_PARAM(cookie, m_cookies, ';', zhao::StringUtil::Trim);
    m_parserParamFlag |= 0x4;
}
...
void HttpResponse::setCookie(const std::string& key, const std::string& val,
    ...
    ss << key << "=" << val;
    if(expired > 0) {
        ss << ";expires=" << zhao::Time2Str(expired, "%a, %d %b %Y %H:%M:%S") << " GMT";
    }
    if(!domain.empty()) {
        ss << ";domain=" << domain;
    }
