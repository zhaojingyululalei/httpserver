...
/**
 * @brief 获取Map中的key值,并转成对应类型,返回是否成功(又获取，又检查)
 * @param[in] m Map数据结构
 * @param[in] key 关键字
...
/**
 * @brief 获取Map中的key值,并转成对应类型（仅仅是获取）
 * @param[in] m Map数据结构
 * @param[in] key 关键字
 * @param[in] def 默认值
 * @return 如果存在且转换成功返回对应的值,否则返回默认值
 */
template<class MapType, class T> //MapType出了httprequest作用域，这里是个模板
...
public:
    ...
    /// MAP结构，http协议头部的key大小写不敏感，只要字母一样就是相同的key，无需区分大小写。
    ...
    void init();
    void initParam();
    /*http_parser只能解析出body header cookies，但是里面的具体键值还得另外解析，通过一下init解析所有键值*/
    void initQueryParam();
    void initBodyParam();
    void initCookies();
private:
